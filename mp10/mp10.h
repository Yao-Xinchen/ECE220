...
/**
 * @brief Initialize a vertex set.
 *
 * Malloc memory for the vertex set, when it is empty.
 * The initial size of the vertex set is 16.
 *
 * @param vs Pointer to the vertex set to be initialized.
 *
 * @return TRUE on success, FALSE on failure.
 */
int vs_init(vertex_set_t *vs);

/**
 * @brief Grow the vertex set if it's full.
 *
 * Judge whether the vertex set is about to be full.
 * If so, double the size of the vertex set.
 * Called in each vs_insert().
 *
 * @param vs Pointer to the vertex set to be grown.
 *
 * @return TRUE on success, FALSE on failure.
 */
int vs_grow(vertex_set_t *vs);

/**
 * @brief Insert a vertex into the vertex set.
 *
 * Insert a vertex into the vertex set in increasing order.
 * The size of the vertex set is tested before insertion.
 *
 * @param vs Pointer to the vertex set.
 * @param id The id of the vertex to be inserted.
 */
void vs_insert(vertex_set_t *vs, int32_t id);

/**
 * @brief Calculate the minimap bit number of a vertex.
 *
 * @param p_id The id of the vertex in the pyramid tree.
 *
 * @return The minimap bit number of the vertex.
 */
