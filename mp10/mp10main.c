...
static request_t *read_request(FILE *f)
{
    ...
    if (6 !=
        (scanres = fscanf(f, "%d%d%d%d%d%d", &r->from.x, &r->from.y, &r->from.range, &r->to.x, &r->to.y, &r->to.range)))
    {
        if (-1 != scanres)
        {
...
static heap_t *make_heap_for_graph(graph_t *g)
{
    heap_t *h;

    if (NULL == (h = malloc(sizeof(*h))) || NULL == (h->elt = malloc(g->n_vertices * sizeof(*h->elt))))
    {
        if (NULL != h)
        {
            free(h);
...
static void free_pyramid_tree(pyr_tree_t *p)
{
    free(p->node);
    free(p);
}
...
static int sort_by_x(const void *p1, const void *p2)
{
    const pos_t *pos1 = p1;
    const pos_t *pos2 = p2;
...
static int sort_by_y(const void *p1, const void *p2)
{
    const pos_t *pos1 = p1;
    const pos_t *pos2 = p2;
...
static void fill_pyramid_tree(pyr_tree_t *p, int32_t nnum, pos_t *pos, int32_t n_vertices, int32_t full)
{
    int32_t rem, part;
    ...
    p->node[nnum].x = pos[left - 1].x + (pos[left].x - pos[left - 1].x) / 2;
    if (1 < left)
    {
        qsort(pos, left, sizeof(*pos), sort_by_y);
        p->node[nnum].y_left = pos[upper_left - 1].y + (pos[upper_left].y - pos[upper_left - 1].y) / 2;
    } else
    {
    ...
    if (upper_left < left)
    {
        fill_pyramid_tree(p, 4 * nnum + 3, pos + upper_left, left - upper_left, full / 4);
    }
    pos += left;
    if (1 < right)
    {
        qsort(pos, right, sizeof(*pos), sort_by_y);
        p->node[nnum].y_right = pos[upper_right - 1].y + (pos[upper_right].y - pos[upper_right - 1].y) / 2;
    } else
    {
    ...
    {
        fill_pyramid_tree(p, 4 * nnum + 4, pos + upper_right, right - upper_right, full / 4);
    }
}


static pyr_tree_t *make_pyramid_tree(graph_t *g)
{
    int32_t tot = 0;
    int32_t amt = 1;
    pos_t *pos;
    ...
    if (NULL == (p = malloc(sizeof(*p))) || NULL == (p->node = malloc(tot * sizeof(*p->node))))
    {
        free(pos);
        if (NULL != p)
        {
...
int32_t find_nodes_inst(locale_t *loc, vertex_set_t *vs, pyr_tree_t *p, int32_t nnum)
{
    fn_count = 0;
...
int32_t show_find_results(graph_t *g, request_t *r1, request_t *r2)
{
    ...
    {
        dx = (g->vertex[r1->path->id[r1->path->n_vertices - 1]].x - g->vertex[r1->path->id[0]].x);
        dy = (g->vertex[r1->path->id[r1->path->n_vertices - 1]].y - g->vertex[r1->path->id[0]].y);
    ...
    fprintf(f, "    draw_circle (%d,%d,%d,%d);\n", WIDTH / 2 + ((int32_t) (scale * (r1->from.x - xmid))),
            HEIGHT / 2 + ((int32_t) (scale * (r1->from.y - ymid))), 0, (int32_t) (scale * r1->from.range));
    fprintf(f, "    draw_circle (%d,%d,%d,%d);\n", WIDTH / 2 + ((int32_t) (scale * (r1->to.x - xmid))),
            HEIGHT / 2 + ((int32_t) (scale * (r1->to.y - ymid))), 0, (int32_t) (scale * r1->to.range));

    fprintf(f, "    set_color (0xFFC080);\n");
    fprintf(f, "    draw_circle (%d,%d,%d,%d);\n", WIDTH / 2 + ((int32_t) (scale * (r2->from.x - xmid))),
            HEIGHT / 2 + ((int32_t) (scale * (r2->from.y - ymid))), 0, (int32_t) (scale * r2->from.range));
    if (show_path)
    {
        // no path -> single request, so use different colors for
        // src and dest
        fprintf(f, "    draw_circle (%d,%d,%d,%d);\n", WIDTH / 2 + ((int32_t) (scale * (r2->to.x - xmid))),
                HEIGHT / 2 + ((int32_t) (scale * (r2->to.y - ymid))), 0, (int32_t) (scale * r2->to.range));
    }
    ...
    if (show_path)
    {
        ...
        for (i = 0, v = r1->path->id[0]; r1->path->n_vertices - 1 > i; i++, v = n)
        {
            n = r1->path->id[i + 1];
            fprintf(f, "    draw_line (%d,%d,%d,%d);\n", WIDTH / 2 + ((int32_t) (scale * (g->vertex[v].x - xmid))),
                    HEIGHT / 2 + ((int32_t) (scale * (g->vertex[v].y - ymid))),
