...
int vs_init(vertex_set_t *vs)
{
    vs->count = 0;
    vs->id_array_size = 16;
    int32_t *ptr = malloc(vs->id_array_size * sizeof(int32_t));
    if (ptr == NULL)
    {
        return FALSE;
    }
...
{
    if (vs->count + 1 >= vs->id_array_size)
    {
        vs->id_array_size *= 2;
        int32_t *ptr = realloc(vs->id, vs->id_array_size * sizeof(int32_t));
        if (ptr == NULL)
        {
            return FALSE;
        }
...
void vs_insert(vertex_set_t *vs, int32_t id)
{
    ...
    int result = vs->count == 0 ? vs_init(vs) : vs_grow(vs);
    if (result == FALSE) return;

    // find the position to insert
    for (int i = 0; i < vs->count; i++)
    {
        if (id < vs->id[i]) // do nothing until vs->id[i] > id
        {
            // move the elements after i to the right
            for (int j = vs->count; j > i; j--)
            {
                vs->id[j] = vs->id[j - 1];
            }
            // insert id at i
    ...
    // if id is the largest, insert it at the end
    vs->id[vs->count] = id;
    vs->count++;
}
